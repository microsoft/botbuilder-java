...
public class EchoBot extends ActivityHandler {
    ...
    protected CompletableFuture<Void> onMessageActivity(TurnContext turnContext) {
        ...
        StatePropertyAccessor<CustomState> dataAccessor = conversationState.createProperty("customState");
        CompletableFuture<CustomState> convStateFuture = dataAccessor.get(turnContext, CustomState::new);

        // Get profile from UserState.
        StatePropertyAccessor<CustomState> profileAccessor = userState.createProperty("customState");
        CompletableFuture<CustomState> userStateFuture = profileAccessor.get(turnContext, CustomState::new);
        ...
        return convStateFuture.thenCombine(userStateFuture, (convProp, userProp) -> {
            ...
            return turnContext.sendActivity(MessageFactory.text(
                String.format("Echo: %s conversation state %d user state %d",

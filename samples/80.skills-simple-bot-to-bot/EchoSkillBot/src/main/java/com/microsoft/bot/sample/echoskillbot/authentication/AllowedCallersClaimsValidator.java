...
public class AllowedCallersClaimsValidator extends ClaimsValidator {
    ...
    public AllowedCallersClaimsValidator(Configuration config) {
        ...
        // that consists of the list of parent bot Ds that are allowed to access the skill.
            ...
            if (!allowedCallers.contains(appId)) {
                return Async.completeExceptionally(
                    new RuntimeException(
                        String.format("Received a request from a bot with an app ID of \"%s\". "
                        +"To enable requests from this caller, add the app ID to your configuration file.", appId)
                    )
                );
            }
        }

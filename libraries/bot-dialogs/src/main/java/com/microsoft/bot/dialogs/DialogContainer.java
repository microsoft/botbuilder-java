...
public abstract class DialogContainer extends Dialog {
     ...
     * When setting this property, all of the contained dialogs' BotTelemetryClient
     ...
     *
     * <p>This override will trace version changes.</p>
     *
    ...
    public CompletableFuture<Boolean> onDialogEvent(DialogContext dc, DialogEvent e) {
        return super.onDialogEvent(dc, e)
            .thenCompose(handled -> {
                ...
                if (!handled && e.getName().equals(DialogEvents.VERSION_CHANGED)) {
                    String traceMessage = String.format(
                        "Unhandled dialog event: {e.Name}. Active Dialog: %s",
                        dc.getActiveDialog().getId()
                    );

                    dc.getDialogs().getTelemetryClient().trackTrace(traceMessage, Severity.WARNING, null);

                    return TurnContext.traceActivity(dc.getContext(), traceMessage)
                        .thenApply(response -> handled);
                }
     ...
     *
     * <p>DialogContainers detect changes of all sub-components in the container and map that to an DialogChanged event.
     ...
     * which is then examined via calls to checkForVersionChange().</p>
     *
    ...
    protected CompletableFuture<Void> checkForVersionChange(DialogContext dc) {
        ...
        if (current != null && current.equals(dc.getActiveDialog().getVersion())) {
            ...
            //   change again.
            return dc.emitEvent(DialogEvents.VERSION_CHANGED, getId(), true, false)

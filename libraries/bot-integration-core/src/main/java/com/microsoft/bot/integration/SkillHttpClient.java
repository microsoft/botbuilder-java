...
public class SkillHttpClient extends BotFrameworkHttpClient {
     ...
     * @param conversationIdFactory  An instance of a class derived from {@link SkillConversationIdFactoryBase}.
     * @param channelProvider        An instance of {@link ChannelProvider}.
     */
    public SkillHttpClient(
     ...
     *
     * @param originatingAudience  The oauth audience scope, used during
     *                             token retrieval. (Either https://api.getbotframework().com or bot app
     *                             id.)
     * @param fromBotId            The MicrosoftAppId of the bot sending
     *                             the activity.
     * @param toSkill              The skill to create the conversation
     *                             Id for.
     * @param callbackUrl          The callback Url for the skill host.
     * @param activity             The activity to send.
     * @param type                 Type of T required due to type erasure of generics in Java.
    ...
    public <T extends Object>  CompletableFuture<TypedInvokeResponse<T>> postActivity(
        ...
        try {
            ...
            skillConversationId =  conversationIdFactory.createSkillConversationId(options).join();
        } catch (Exception ex) {
            // Attempt to create the ID using deprecated method.
            skillConversationId =  conversationIdFactory.createSkillConversationId(
                                                            activity.getConversationReference()).join();
        }

        return  postActivity(fromBotId,
                             toSkill.getAppId(),
                             toSkill.getSkillEndpoint(),
                             callbackUrl,
                             skillConversationId,
                             activity,
                             type);
